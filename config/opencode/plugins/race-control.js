...
var DEFAULT_PLUGIN_URL =
	"http://192.168.1.253:8317/v0/resource/plugins/race-control/opencode-plugin.js";
var DEFAULT_MODELS_URL =
	"http://192.168.1.253:8317/v0/resource/plugins/race-control/models.json";
var CACHE_PATH = join(
	homedir(),
	".cache",
	"opencode",
...
async function refreshCache(pluginURL) {
	...
	if (!response.ok) {
		throw new Error(
			`Race Control plugin download failed with status ${response.status}`,
		);
	}
