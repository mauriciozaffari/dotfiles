...
Keep Stimulus controllers small. Reusable controllers are good for generic behaviors; domain-specific controllers are fine when the interaction belongs to a particular screen.
...
Tailwind is welcome when it keeps styling fast and consistent. Keep class names static and discoverable so the compiler can see them.
