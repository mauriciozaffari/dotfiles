# Architecture - Develoz Rails Style
...
Do not prescribe one authentication library for every app. Follow the application's existing auth approach and keep session/token lifecycle explicit and well tested.
...
Good authentication code is:
- explicit about session/token creation and expiration
- covered by request specs for sign-in, sign-out, password/reset or magic-link flows
- boring about security primitives
- consistent with the app's chosen library or custom implementation
...
**SolidQueue by default:**
- Good Rails 8 default
- Low operational complexity
- Same transactional guarantees as your data

Sidekiq and Redis are acceptable when throughput, tooling, or existing infrastructure justify them.
...
**Caching backend:**
- Solid Cache is a good default when database-backed caching is enough
- Redis is acceptable when latency, sharing, or existing infrastructure justify it
- HTTP and fragment caching should still be considered before backend complexity
</caching>
...
**RSpec and FactoryBot by default:**

```ruby
RSpec.describe Card do
  it "creates a closure when closed" do
    card = create(:card)

    card.close

    expect(card).to be_closed
    expect(card.closure).to be_present
  end
end
```

**Factories for local setup:**

```ruby
card = create(:card, :open, board: board, creator: user)
```

**Request specs** for controllers:

```ruby
RSpec.describe "Card closures" do
  it "closes a card" do
    card = create(:card)
    sign_in_as(card.creator)
